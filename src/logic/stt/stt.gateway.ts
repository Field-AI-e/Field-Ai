...
export class SttGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    private clientSessions = new Map<
        string,
        {
            ...
            lastSpeech: number;
            lastFinalTranscript: number; // Track when last final transcript was received
            lastPartialTranscript: number; // Track when last partial transcript was received
            interval: NodeJS.Timeout;
            voiceDetectionInterval: NodeJS.Timeout; // Separate interval for voice detection
            elevenConnection: RealtimeConnection;
            pauseEmitted: boolean; // Prevent multiple pause emissions
            voiceStoppedEmitted: boolean; // Prevent multiple voice-stopped emissions
        }
    >();
    ...
    handleDisconnect(client: Socket) {
        ...
        if (session.voiceDetectionInterval) {
            clearInterval(session.voiceDetectionInterval);
        }
